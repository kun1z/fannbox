...
si main(void)
{
    ...
    {
        const r32 error = threads > 1 ? fann_train_epoch_irpropm_parallel(ann, data, threads) : fann_train_epoch(ann, data);

        if (!(i % epochs_between_reports) || i == max_epochs || i == 1)
        {
            printf("Epochs %12u. Current error: %.10f.\n", i, error);
        }
